    ...
    const response = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayload) => {
        ...
        if (user.deletedAt || user.suspendedAt) {
          throw new Error("This account cannot perform marketplace actions");
        }
        if (!pathname.startsWith("listing-covers/")) {
          throw new Error("Invalid listing cover path");
        }
        return {
      ...
      onUploadCompleted: async ({ blob, tokenPayload }) => {
        ...
        console.info("listing-cover.uploaded", {
